...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, reacts, save_group_settings, import_site, get_shortlink, send_all, check_verification, get_token, import_site, get_text, imdb
...
async def stream_download(bot, query):
    ...
    if not await db.has_premium_access(user_id) and JK_STREAM_MODE is True:
...
async def reply_stream(client, message):
    ...
    if not await db.has_premium_access(user_id) and JK_STREAM_MODE == True:  
...
async def force_sub(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        msg=await message.reply_photo(
            photo="https://graph.org/file/f8961b3f8260d9b4819f1.jpg", caption=f"<b>⚠ Dᴇᴀʀ Usᴇʀ {message.from_user.mention}!\n\nᴛᴏ ɢᴇᴛ ᴍᴏᴠɪᴇs ᴅɪʀᴇᴄᴛʟʏ, ʏᴏᴜ ʜᴀᴠᴇ ᴛᴏ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ ғɪʀsᴛ 🥶\n\n ᴀғᴛᴇʀ ᴊᴏɪɴɪɴɢ ᴄᴀᴍᴇ ʙᴀᴄᴋ ᴀɴᴅ sᴇᴀʀᴄʜ ᴀɢᴀɪɴ 👻\n\n<i>पहले आप हमारे चैनल को ज्वाइन करे फिर वापस आकर फिर से सर्च करें✍</i></b>",
...
async def pm_text(bot, message):
    ...
    if not PM_FILTER and not await db.has_premium_access(message.from_user.id):
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton('🤖 ʙᴜʏ ʙᴏᴛ ᴘᴍ ᴘʀᴇᴍɪᴜᴍ 🎗️', callback_data=f'seepl'),[InlineKeyboardButton('💫 Request Here', url="https://t.me/+5mkRHNxvgT1lNjNl")]]))
...
async def pm_spoll_choker(bot, query):
    ...
    if files:
        ...
        try:
            ...
            if NO_RESULTS_MSG:
                safari = [[
                    InlineKeyboardButton('Not Release 📅', callback_data=f"not_release:{reqstr1}:{search}"),
                ],[
                    InlineKeyboardButton('Already Available🕵️', callback_data=f"already_available:{reqstr1}:{search}"),
                    InlineKeyboardButton('Not Available🙅', callback_data=f"not_available:{reqstr1}:{search}")
                ],[
                    InlineKeyboardButton('Uploaded Done✅', callback_data=f"uploaded:{reqstr1}:{search}")
                ],[
                    InlineKeyboardButton('Series Error🙅', callback_data=f"series:{reqstr1}:{search}"),
                    InlineKeyboardButton('Spell Error✍️', callback_data=f"spelling_error:{reqstr1}:{search}")
                ],[
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id) and not await db.has_premium_access(query.from_user.id):
                        btn = [[
                            ...
                            InlineKeyboardButton("😄 ɴᴏ ɴᴇᴇᴅ ᴛᴏ ᴠᴇʀɪғʏ 😄", callback_data='seeplans')
                        ], [
                        ...
                        return await query.answer('मूवी आपको प्रस्नल चैट में भेजा हूं', show_alert=True)
                else:
